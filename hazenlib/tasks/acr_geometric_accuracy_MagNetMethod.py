...
class ACRGeometricAccuracyMagNetMethod(HazenTask):
    ...
    def run(self) -> dict:
        ...
        horz_CoV_mm, vert_CoV_mm = self.get_rod_CoV(horz_distances, vert_distances)

        correction_coefficients_mm = self.get_rod_distortion_correction_coefficients(horizontal_distances=horz_distances)

        Avg_horizontal_Distance_mm = np.mean(horz_distances) * self.pixel_size
        ...
        if self.report:
            import matplotlib.pyplot as plt
            plt.close("all")
            self.plot_rods(plt.gca(), arr, rods, rods_initial,horz_distances_mm,vert_distances_mm)
            img_path = os.path.realpath(os.path.join(self.report_path, f"{self.img_desc(slice5_dcm)}.png"))
            plt.savefig(img_path)
            self.report_files.append(img_path)
            

        distortion_values = {
            "vertical CoV": round(vert_CoV_mm, 2),
            "horizontal CoV": round(horz_CoV_mm, 2),
        }

        linearity_values = {
            "Mean Vertical Distance mm": round(Avg_vertical_Distance_mm, 2),
            "Mean Horizontal Distance mm": round(Avg_horizontal_Distance_mm, 2),
        }
...
#                plt.show()           
    ...
    def plot_rods(self, ax, arr, rods, rods_initial,HorDist,VertDist):  # pragma: no cover
        ...
        for i in range(0,9,3):
            plt.plot([rods[i].x,rods[i+2].x],[rods[i].y,rods[i+2].y],color=color[count])
        ...
        for i in range(0,3):
            MidPointx = (rods[i].x+rods[i+6].x)/2
            MidPointy = (rods[i].y+rods[i+6].y)/2
        ...
        ax.set_title("Rods and Distances")
        return ax
    ...
    def get_rod_CoV(self, horz_dist_mm, vert_dist_mm):
        """
